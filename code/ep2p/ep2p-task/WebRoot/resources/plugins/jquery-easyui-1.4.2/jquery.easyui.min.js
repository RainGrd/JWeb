...
var _482=tb.parent();
/*opts.height=28;
if($(_47f).hasClass('textbox230')||$(_47f).hasClass('combobox230')||$(_47f).hasClass('numberbox230')){
	opts.width=220;
}else if($(_47f).hasClass('multtextbox554')){
	opts.width=544;
	opts.height=55;
}else if($(_47f).hasClass('textbox580')){
	opts.width=580;
}else{
	if(_482.hasClass('fieldDiv200')){
		opts.width=190;
	}else if(_482.hasClass('fontDiv85')){
		opts.width=83;
	}else if(_482.hasClass('fieldDiv554')){
		opts.width=544;
	}else if(_482.hasClass('fieldDiv235')){
		opts.width=220;
	}else if(_481.options.width!='auto'){
		opts.width=_481.options.width-2;
	}else{  // update by zhengxiu 2015/0703
	//opts.width=148;
		opts.width=168;}
}*/
opts.width=168;
...
if(opts.pagination){
// $.extend(_6e3,{page:opts.pageNumber||1,rows:opts.pageSize}); find by wzx
...
// update by zhengxiu 2015/06/15 start 
// cc.push(col.checkbox?"class=\"datagrid-cell-check\"":" class=\"datagrid-cell "+col.cellClass+"\"");	
//监理单位,施工单位,设计单位,建设单位,隧道名称,涵洞名称,桥梁名称,备注，养护单位 20150625 修改 把所有列表字段加...
/* if(_75e=="supervisionCompany"||_75e=="constructionCompany"||_75e=="designCompany"||_75e=="buildCompany"||_75e=="tunnelName"||_75e=="culvertName"||_75e=="bridgeName"||_75e=="remark"||_75e=="maintainCompanyName"||_75e=='checkReason'||_75e=='dmessagedesc'||_75e=='processsuggestion'||_75e=='itemName'||_75e=='damageDesc'){
	cc.push(col.checkbox?"class=\"datagrid-cell-check\"":" title=\""+_75f+"\" class=\"ellipsis-cell datagrid-cell "+col.cellClass+"\"");	
}else{
	cc.push(col.checkbox?"class=\"datagrid-cell-check\"":" class=\"datagrid-cell "+col.cellClass+"\"");	
} */
var cTitle=_75f;
if(_75e=="km"){
	cTitle=col.formatter(_75f,_75c,_75b);
}
if(cTitle!=''&&cTitle!=undefined&&_75e!='isDisease'){
	if(_75e=="buildDt"||_75e=='checkDttm'||_75e=='cretedate'||_75e=='changeDt'||_75e=='creDttm'||_75e=='usedDt'||_75e=='signInDttm'){
		// 判断是否存在数据  如果不存在，直接退出方法 
		if(null == cTitle || "" == cTitle){
			return "";
		}
		if(isNaN(cTitle)){ //如果不是时间戳格式
			// 去掉时间后面的  .加数字 
			var index = cTitle.indexOf(".");
			// 如果不存在,那index就等于总长度
			if(index == -1){
				index = cTitle.length;
			}
			// 截取时间字符串,并转换
			var str=cTitle.substring(0,index).toString();
			// 把时间里面的  -  转换城 /
			str = str.replace(/-/g,"/");
		}else{//时间戳格式
			str=cTitle;
	  ...
	  var d = date.getDate();
	  // 返回指定的时间格式
	  cTitle=y+'-'+(m<10?('0'+m):m)+'-'+(d<10?('0'+d):d);
	}else if(_75e=="dmessagedesc"){
		cTitle = cTitle.replace(/<br\s*\/?>/g,";");
	}
	cc.push(col.checkbox?"class=\"datagrid-cell-check\"":" title=\""+cTitle+"\" class=\"ellipsis-cell datagrid-cell "+col.cellClass+"\"");	
}else{
	cc.push(col.checkbox?"class=\"datagrid-cell-check\"":" class=\"datagrid-cell "+col.cellClass+"\"");	
}
// update by zhengxiu 2015/06/15 end
cc.push(">");
if(col.checkbox){
