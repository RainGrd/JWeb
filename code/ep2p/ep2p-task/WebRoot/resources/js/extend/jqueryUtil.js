(function ($)
{
	...
	//修改ajax默认设置
		...
		/**
		 * 公共的分页查询,封装form表单属性
		 * datagrid datagrid列表的ID
		 * formId form表单ID
		 */
		jqueryUtil.ajaxSearchPage = function(datagrid,formId){
			// 重新加载数据
			$(datagrid).datagrid('load', jqueryUtil.serializeObject($(formId),true));
			// 清空所选择的行数据
...
jQuery.download = function(url, data, method){   
	...
	if( url && data ){ 
	    ...
	    // request发送请求
	    jQuery('<form action="'+ url +'" method="'+ (method||'post') +'" accept-charset="UTF-8" enctype="UTF-8">'
	    		+inputs+'</form>')
