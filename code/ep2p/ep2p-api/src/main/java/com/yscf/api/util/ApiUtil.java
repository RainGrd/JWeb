...
public class ApiUtil {
	...
	public static String getEncryptStr(JsonObject jsonobject){
		...
		if(StringUtil.isNotEmpty(gsonObj)){
			FileConfig config = (FileConfig)SpringApplicationContext.getBean("fileConfig");
			regson = gzipCompress(SecurityUtil.encrypt(config.getEncryptKey(), gsonObj));
		}
		return regson;
	}
	...
	public static String getHeadXtokenDecryptStr(HttpServletRequest request){
		String reStr = null;
		//获取头部信息中的关键信息
		String xtoken = request.getHeader(ConstantApi.TOKEN);
	 ...
	 * @return
	 */
	public static String getHeadTranPKeyDecryptStr(HttpServletRequest request){
		String reStr = null;
		//获取头部信息中的关键信息
		String tranPKey = request.getHeader(ConstantApi.TRANPKEY_KEYVAL);
		if(StringUtil.isNotEmpty(tranPKey)){
			FileConfig config = (FileConfig)SpringApplicationContext.getBean("fileConfig");
			reStr = SecurityUtil.decrypt(config.getEncryptKey(), tranPKey);
		}
		return reStr;
	}
	 ...
	 * @return
	 */
	public static Object convertObjectByDecryptStr(String decryptStr,Class<?> clazz){
	 ...
	 * @return
	 */
	public static Object convertObjectByBody(HttpServletRequest request,Class<?> clazz){
	 ...
	 * 
	 * Description：<br> 
	 * 将密文解析后，转成对应的java对象，主要用户解析API接口参数
	 * 注意：密文必须是JSON格式通过加密后的字符串，否则转换会失败
	 * 如：
	 * 加密前：{"id":"1","name":"小杰","age":"20"}
	 * 加密后：	e3d519b6a30d28d966b8f3ce69fe9570847c261d665c2d6320859e2235ca2403962b0d57a176aa7c
	 * @author  JunJie.Liu
	 * @date    2016年1月5日
	 * @version v1.0.0
	 * @param encryptStr
	 * 			密文： e3d519b6a30d28d966b8f3ce69fe9570847c261d665c2d6320859e2235ca2403962b0d57a176aa7c
	 * @param clazz
	 * 			将要转成的对象  
	 * @return
	 */
	public static Object convertObjectByEncryptStr(String encryptStr,Class<?> clazz){
		...
		FileConfig config = (FileConfig)SpringApplicationContext.getBean("fileConfig");
		return gson.fromJson(SecurityUtil.decrypt(config.getEncryptKey(),encryptStr), clazz);
	}
