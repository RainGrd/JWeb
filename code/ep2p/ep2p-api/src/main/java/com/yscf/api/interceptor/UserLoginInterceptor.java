...
public class UserLoginInterceptor implements HandlerInterceptor  {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		// token 为空，需要APP端传入token值，否则不能访问
		if(StringUtil.isEmpty(token)){
			bool = false;
		}else{
			// 从缓存数据中获取不到，说明session已失效，需要重新登陆
			CustomerVo custvo = memcachedClient.get(token);
			if(null == custvo){
				bool = false;
			}
		}
		...
		if(!bool){
			...
			jsonobject.setCode(ApiCode.HTTP_CODE_905);
			jsonobject.setStatus(false);
			jsonobject.setMessage("token已失效或用户未登录");
