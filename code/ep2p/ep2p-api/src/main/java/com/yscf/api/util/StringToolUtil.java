...
public class StringToolUtil {
	 ...
	 * @return
	 */
	public static Map<String, Object> checkVersion(String s1, String s2) {
		
		Map<String, Object> map = Maps.newHashMap();
		map.put("isCompelUp", false);
		map.put("isUp", false);
		if(!isEmpty(s1)&&!isEmpty(s2)){
			String[] arr1 = s1.split("[^a-zA-Z0-9]+"), arr2 = s2.split("[^a-zA-Z0-9]+");
			if(arr1.length==3||arr2.length==3){
				int v11 = asInteger(arr1[0]);int v21=asInteger(arr2[0]);
				int v12 = asInteger(arr1[1]);int v22=asInteger(arr2[1]);
				int v13 = asInteger(arr1[2]);int v23=asInteger(arr2[2]);
				if(v11>v21){
					map.put("isCompelUp", true);
					map.put("isUp", true);
					return map;
				}else{
					if(v11==v21){
