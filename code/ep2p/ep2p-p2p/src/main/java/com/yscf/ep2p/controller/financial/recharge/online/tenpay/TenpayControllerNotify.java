 ...
 *************************************************************************
 */
package com.yscf.ep2p.controller.financial.recharge.online.tenpay;
 ...
 */
@Controller
@RequestMapping("recharge/tenpayNotify")
public class TenpayControllerNotify extends PayServletController {
	
	private final Logger logger = LoggerFactory.getLogger("ten");
	
	@Autowired
	public TenpayControllerNotify(BizRechargeOnlineServiceImpl service) {
		super(service);
	}

	@Override
	protected String getPaySystemType() {
		return Constant.TEN_PAY;
	}
	...
	protected void postHandle(HttpServletRequest req, HttpServletResponse resp,
		...
		if(result != Constant.PAY_UNDEFINED){
			this.logger.info("server notify OK: " + feedbackData.get("out_trade_no"));
