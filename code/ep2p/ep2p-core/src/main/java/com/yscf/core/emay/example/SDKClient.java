package com.yscf.core.emay.example;

import java.rmi.RemoteException;
import java.util.Iterator;
import java.util.List;

import com.yscf.core.emay.sdkhttp.Mo;
import com.yscf.core.emay.sdkhttp.StatusReport;

public class SDKClient {
	public static String softwareSerialNo = "9SDK-EMY-0999-JDUUP";// 软件序列号,请通过亿美销售人员获取
	public static String key = "ep2p2013";// 序列号首次激活时自己设定
	public static String password = "808798";// 密码,请通过亿美销售人员获取
...
//		Client client=SDKClient.getClient();
	 ...
	 * 3、addSerial 附加码(长度小于15的字符串) 用户可通过附加码自定义短信类别,或添加自定义主叫号码( 联系亿美索取6主叫号码列表)
	 * 4、优先级范围1~5，数值越高优先级越高(相对于同一序列号) 5、其它短信发送请参考使用手册自己尝试使用
	 */
	public static void testSendSMS() {
		try {
			int i = SingletonClient.getClient().sendSMS(new String[] { "15002008095" }, "【e生财富】张雨测试短信发送。。。^_^ ", "",5);// 带扩展码
	...
	public static void testSendScheduledSMS() {
		try {
			int i = SingletonClient.getClient().sendScheduledSMSEx(new String[] { "18219528634" },
					"【e生财富】异步内容", "20110430174830", "GBK");
	...
	public static void testSendSMSAddMessageId() {
		try {
			int i = SingletonClient.getClient().sendSMSEx(new String[] { "18219528634" }, "带有信息ID的短信", "", "GBK", 5, 123456789);
			System.out.println("testsSendScheduledSMS=====" + i);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	*/
	...
	public static void testSendVoice() {
		try {
			String value = SingletonClient.getClient().sendVoice(new String[] { "18219528634" }, "123456", "", "GBK", 5, System.currentTimeMillis());
