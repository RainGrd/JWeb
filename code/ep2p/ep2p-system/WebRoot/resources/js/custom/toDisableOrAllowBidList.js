var cusBidStatus={
		...
		init:function(){
			var pid = $("#pid").val();
			cusBidStatus.loadRadio();
		},
		//初始化
		initDataGrid:function(){
			var pid = $("#pid").val();
			$('#showCusGrids').datagrid({     
				url:basePath + 'customerController/selectBidCusStatusHistoryById.shtml',
				width:'100%',
				method:'post',
				queryParams:{'pid':pid},
				fit:true,
			    ...
			    sortOrder:'asc',
			    remoteSort:false,
			    columns:disOrEnable_Bid_Cus_Status,
		...
		save:function(){
			...
			$.ajax({
				type: "POST",
		        async:false, 
		    	url : BASE_PATH+"customerController/saveBidCustomerStatus.shtml",
		    	data:{"data":objStr},
				dataType: "json",
			    success: function(data){
			    	if(data.message.status == 200 ){
			    		$.messager.alert('操作提示',"保存成功！",'success');
			    		window.location.href=BASE_PATH+"customerController/toDisableOrAllowBidList.shtml?pid="+pid;
			    	}else{
		...
		loadRadio:function(){
			$.post(BASE_PATH+"sysDistionaryContentController/selectByDisctCodeNoPlease.shtml",{"dictCode":"CUA_BID_STATUS"}, 
				function(ret) {
