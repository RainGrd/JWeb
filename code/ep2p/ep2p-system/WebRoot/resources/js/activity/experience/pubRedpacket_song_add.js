...
var pubRedpacket_song_add = {
	 				...
	 				if(result.message == 200){
	 					...
	 					if($("#pid").val() == -1){
		 					$.messager.alert('操作提示','新增成功','info',function(){
	 							reloadTabGrid("送红包活动设置");
		 						// 关闭当前页面
		 						parent.$('#centerFrame').tabs('close',"新增送红包活动");
	 						});
	 					}else{
	 						$.messager.alert('操作提示','编辑成功','info',function(){
	 							reloadTabGrid("送红包活动设置");
		 						// 关闭当前页面
		 						parent.$('#centerFrame').tabs('close',"编辑送红包活动");
	 						});
	 					}
	 				}else{
		...
		initPubRedpacket_add:function(){
			$("#baseInfo").form("reset");
			commonActivity.generatNoRuleTem("SHB","7",4);
			// 初始化条件列表
...
$(function(){
	...
	if(pid == -1){
		// 如果是新增  == -1
		pubRedpacket_song_add.initPubRedpacket_add();
	}else{
		// 如果是编辑  == 其他
		pubRedpacket_song_add.initPubRedpacket_edit(pid,2);
	}
});