...
var pubRedpacket_model = [[    
    	        ...
    	        {field:'redpacketType',title:'红包类型',width:'10%',align:'center',sortable:true,formatter:formatRedpacketType},   
    	        {field:'repNumber',title:'红包个数',width:'8%',align:'center',sortable:true},    
    	        {field:'repAmount',title:'红包金额',width:'10%',align:'right',sortable:true,formatter:common.formatCurrency},
    	        {field:'allotType',title:'分配方式',width:'8%',align:'center',sortable:true,formatter:formatAllotType}, 
 ...
 * @returns
 */
function formatAllotType(value,row,index){
	if(row.allotType == 1){
		return "平均分配";
	}else if(row.allotType == 2){
		return "随机分配";
	}
	return "未知";
}
...
function formatCaoZuo(value,row,index){
	// 跳转到编辑页面
	var path = BASE_PATH + "pubRedpacketController/openPubRedpacketAdd.shtml?pid="+row.pid;
	var str = "编辑抢红包活动";
